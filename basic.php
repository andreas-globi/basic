...
class Basic {
    ...
    public function preinterpret($source) {
        self::$md5 = md5($source);
        // Tokenise
    ...
    public function getStateKey($md5, $id = 0) {
        return 'state:' . $md5 . ':' . $id;
    }

    public function loadState($redis, $id = 0) {
        $key = $this->getStateKey(self::$md5, $id);
        $resp = $redis->get($key);
        if ($resp) {
            $state = json_decode($resp);
            if ($state) {
                self::$current_statement = $state['current_statement'];
                self::$variables = $state['variables'];
            }
        }
    }

    public function saveState($redis, $id = 0, $debug = false) {
        $key = $this->getStateKey(self::$md5, $id);
        $state = [
        ...
        if ($debug) echo "saving state $json to $key\n";
        $redis->set($key, $json);
    }

    /**
     * @return bool can continue
     */
    public function interpretStatement() {
        if (self::$current_statement >= count(self::$statements)) {
            return false;
        }
        self::$statements[self::$current_statement]->execute();
        self::$current_statement++;
        return self::$current_statement < count(self::$statements);
    }
...
class Parser {
    ...
    public function parse() {
        ...
        while (TRUE) {
            $cur = $this->current();
            // Is this a label?
            if ($this->match(self::TOKEN_LABEL)) {
            ...
            // Is it an assignment?
            else if ($this->match(self::TOKEN_WORD, self::TOKEN_EQUALS)) {
            ...
            // Is it an input statement?
                ...
                $statements[] = new InputStatement($this->next_token(self::TOKEN_WORD)->token);
                $this->position++;
                $this->position++;
            }
            
            // Is it a goto statement?
            else if ($cur && $cur->token == "goto") {
                // Similar to above, get the next token (label to go to) and create new goto statement
                $statements[] = new GotoStatement($this->next_token(self::TOKEN_WORD)->token);
                $this->position++;
                $this->position++;
            }
            
            // Is it an if statement?
            else if ($cur && $cur->token == "if") {
                ...
                $label = $this->next_token(self::TOKEN_WORD)->token;
                $this->position++;
                $this->position++;
    ...
    public function operator() {
        ...
        while ($this->match(self::TOKEN_OPERATOR) || $this->match(self::TOKEN_EQUALS)) {
            // Get the operator
